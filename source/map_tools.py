...
def bounce_twin(POS,CNT,trs=1.65):
	for cbt in range(CNT):
		pO=.32*cbt
		c.spawn(ID=3,p=(POS[0]+pO,POS[1],POS[2]))
		c.spawn(ID=3,p=(POS[0]+pO,POS[1]+trs,POS[2]))
	del POS,CNT,trs

def steel_bridge(POS,CNT):
	for cst in range(CNT):
		c.spawn(ID=0,p=(POS[0]-.32*cst,POS[1],POS[2]))
	del POS,CNT
...
def crate_row(ID,POS,CNT,WAY,l=None,m=None):# WAY: 0=right,1=front,2=up
	for cro in range(CNT):
		...
		if WAY == 0:
			if ID == 13:
				c.spawn(ID=ID,p=(POS[0]+pO,POS[1],POS[2]),m=m,l=l)
			else:
				c.spawn(ID=ID,p=(POS[0]+pO,POS[1],POS[2]))
		elif WAY == 1:
			if ID == 13:
				c.spawn(ID=ID,p=(POS[0],POS[1],POS[2]+pO),m=m,l=l)
			else:
				c.spawn(ID=ID,p=(POS[0],POS[1],POS[2]+pO))
		else:
			if ID == 13:
				c.spawn(ID=ID,p=(POS[0],POS[1]+pO,POS[2]),m=m,l=l)
			else:
...
def crate_wall(ID,POS,CNT):#[x,y]
	for cwX in range(CNT[0]):
		for cwY in range(CNT[1]):
			c.spawn(ID=ID,p=(POS[0]+.32*cwX,POS[1]+.32*cwY,POS[2]))
...
def crate_stair(ID,POS,CNT,WAY):# WAY: 0=up,1=down
	for cs in range(CNT):
		...
		c.spawn(ID=ID,p=(POS[0]+pO,c_way[WAY],POS[2]))
	...
	for cbX in range(CNT[0]):
		for cbZ in range(CNT[1]):
			for cbY in range(CNT[2]):
				...
				pA=.32*cbY
				pE=.32*cbZ
				c.spawn(ID=ID,p=(POS[0]+pO,POS[1]+pA,POS[2]+pE))
	del ID,POS,CNT,cbX,cbZ,cbY,pO,pA,pE

def crate_plane(ID,POS,CNT):#[x,z]
	for cwX in range(CNT[0]):
		for cwZ in range(CNT[1]):
			c.spawn(ID=ID,p=(POS[0]+.32*cwX,POS[1],POS[2]+.32*cwZ))
